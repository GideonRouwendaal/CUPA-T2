...
from data.t2star_loader import generate_and_apply_masks
...
def process_input_data(device, data, add_mean_fs=False):
    """Processes input data for training."""

    img_cc_fs = data[0].to(device)
    sens_maps = data[1].to(device)
    brain_mask = data[2].to(device)
    filename, slice_num = data[3:5]
    random_mask = [detach_torch(item[0]) for item in data[5]]

    if isinstance(data[6][0], torch.Tensor):
        var_acc_rate = [detach_torch(item[0]) for item in data[6]]
    else:
        var_acc_rate = detach_torch(data[6][0])
    ...
    mask, img_cc_zf = generate_and_apply_masks(detach_torch(img_cc_fs),
                                               detach_torch(sens_maps),
                                               mask_configuration=random_mask,
                                               var_acc_rate=var_acc_rate,
                                               add_mean_fs=add_mean_fs)
    mask, img_cc_zf = mask.to(sens_maps.device), img_cc_zf.to(sens_maps.device)
...
def bin_excl_rate(excl_rate):
    """Bin the exclusion rate into bins 0-0.05, ..., 0.45-0.5."""

    bins = np.arange(0, 0.60, 0.05)
    binned_excl_rate = np.digitize(detach_torch(excl_rate), bins)
    binned_excl_rate = (torch.tensor(
        (bins[binned_excl_rate - 1] + bins[binned_excl_rate]) / 2
    ))
    return binned_excl_rate.to(excl_rate.device).to(excl_rate.dtype)
...
def generate_uncertainty_predictions(model, hyper_setup, img_cc_zf, kspace_zf, mask, sens_maps, T=100, excl_rate=False):
    # Turn on train for MC Dropout
    model.eval()
    ...
    for _ in range(T):
        with torch.no_grad():
            if hyper_setup:
                pred = model(excl_rate, (img_cc_zf, kspace_zf, mask, sens_maps))
            else:
                pred = model(img_cc_zf, kspace_zf, mask, sens_maps)
            predictions.append(pred)
    predictions = torch.stack(predictions, dim=0)
    
    mean, std = torch.mean(predictions, dim=0), torch.std(predictions, dim=0)
    model.eval()
    return [mean, std, predictions]


def forward_pass(model, hyper_setup, img_cc_zf, kspace_zf, mask, sens_maps,
                 continuous_excl_rate=True, uncertainty=False, T=100):
    """Forward pass of either HyperNetwork or normal network."""
    
    if hyper_setup:
        excl_rate = torch.unique(
            1.0 - torch.mean(mask,  dim=[1, 2, 3, 4]).to(torch.float32)
        )
        if excl_rate.shape[0] > 1:
            raise ValueError(
                "The rate of excluded lines is not the same for all "
                "samples in the batch: {}".format(excl_rate))

        if not continuous_excl_rate:
            excl_rate = bin_excl_rate(excl_rate)

        if uncertainty:
            prediction = generate_uncertainty_predictions(model, hyper_setup, img_cc_zf, kspace_zf, mask, sens_maps, T)
        else:
            prediction = model(excl_rate, (img_cc_zf, kspace_zf, mask, sens_maps))
    else:
        if uncertainty:
            prediction = generate_uncertainty_predictions(model, hyper_setup, img_cc_zf, kspace_zf, mask, sens_maps, T)
        else:
            prediction = model(img_cc_zf, kspace_zf, mask, sens_maps)
    ...
    for descr in metrics_to_be_calc:
        for m in methods_dict:
            if descr.startswith(m):
                ...
                break
        metrics[descr] = metric(
            np.abs(detach_torch(target)),
            np.abs(detach_torch(data)),
...
def log_images_to_wandb(prediction_example, ground_truth_example,
                        ...
                        uncertainty_map_example=None,
                        wandb_log_name="Examples", count=0,
    ...
    if captions is None:
        captions = ["Reconstruction", "Ground truth", "Zero-filled", "Mean", "Uncertainty", "Uncertainty Normalized"]
    data_operations = {
        "magn": np.abs,
        "phase": np.angle,
        "real": np.real,
    ...
    for data_type in data_types:
        ...
        if prediction_example.shape[0] > 1:
            ...
            if mask_example is not None:
                mask = mask_example[:, 0]
                mask = convert2wandb(mask, np.ones(12), np.zeros(12),
                                     media_type="video",
                                     caption="Mask")
                to_log.append(mask)

            if uncertainty_map_example is not None:
                # normalized_uncertainty = convert2wandb(normalized_uncertainty,
                #                                         np.ones(12), np.zeros(12),
                #                                         media_type="video",
                #                                         caption="Uncertainty Normalized")
                unc = uncertainty_map_example  # [12, 92, 112]
                unc_norm = (unc - unc.min()) / (unc.max() - unc.min() + 1e-6)  # [0,1] per voxel

                red_unc_rgb = np.stack([
                    unc_norm,  # Red channel
                    np.zeros_like(unc_norm),  # Green
                    np.zeros_like(unc_norm)   # Blue
                ], axis=1) * 255.0  # Scale to 0-255
                red_unc_rgb = red_unc_rgb.astype(np.uint8)

                to_log.append(
                    wandb.Video(red_unc_rgb, fps=0.5, caption=captions[4])  # Direct RGB video
                )

                pred_norm = (pred_data - pred_data.min()) / (pred_data.max() - pred_data.min() + 1e-6)  # [0,1] per voxel
                pred_rgb = np.stack([pred_norm] * 3, axis=1) * 255  # (12,3,H,W)
                pred_rgb = pred_rgb.astype(np.uint8)

                alpha = unc_norm[..., None].transpose(0, 3, 1, 2)  # (12,1,H,W)
                overlay = (1 - alpha) * pred_rgb + alpha * red_unc_rgb
                overlay = np.clip(overlay, 0, 255).astype(np.uint8)

                to_log.append(
                    wandb.Video(overlay, fps=0.5, caption=captions[5])  # Blended overlay video
                )

                # mean_img = convert2wandb(mean_img, max_value, min_value,
                #                          media_type="video",
                #                          caption=captions[3])
                # uncertainty_map = convert2wandb(uncertainty_map_example,
                #                                 np.ones(12), np.zeros(12),
                #                                 media_type="video",
                #                                 caption=captions[4])

                # overlay = convert2wandb(normalized_uncertainty * 255.0,
                #                                 np.ones(12), np.zeros(12),
                #                                 media_type="video",
                #                                 caption=captions[5])

        log_key = "{}{}/{}".format(wandb_log_name, data_type, count)

        wandb.log({log_key: to_log})
...
def convert2wandb(data, abs_max_value, min_value, media_type="video",
    ...
    if media_type == "video":
        if np.amin(min_value) < 0:
            # Swap axes, normalize, scale to 0-255
            video_gray = ((np.swapaxes(data[:, None], -2, -1)
                        / abs_max_value[:, None, None, None] + 1) * 127.5
                        ).astype(np.uint8)
            if video_gray.shape[1] == 1:
                video_gray = video_gray.repeat(3, axis=1)

            return wandb.Video(video_gray, fps=0.5, caption=caption)
        else:
            video_gray = (np.swapaxes(data[:, None], -2, -1)
                        / abs_max_value[:, None, None, None] * 255
                        ).astype(np.uint8)
...
def plot2wandb(maps, ref_map, titles, wandb_log_name):
    """
    ...
    maps = [detach_torch(map_i) for map_i in maps]
    ref_map = detach_torch(ref_map)

    fig = plt.figure(figsize=(1.5*(len(maps)+1), 3), dpi=300)
    min_value = np.amin([*maps, ref_map])
    max_value = np.amax([*maps, ref_map])
    for nr, (map_i, title) in enumerate(zip([*maps, ref_map], titles)):
        plt.subplot(2, len(maps)+1, nr + 1)
    ...
    min_value = np.amin([abs(map_i - ref_map) for map_i in maps])
    max_value = 70
    for nr, map_i in enumerate(maps, start=len(maps)+1):
        plt.subplot(2, len(maps)+1, nr + 1)
        plt.imshow(abs(map_i - ref_map).T, vmin=min_value, vmax=max_value)
        plt.axis("off")
        ...
        for a in agg:
            if a == "moco_mean":
                files = sorted(
                    glob.glob(
                        "{}{}/{}/{}/{}_predictions/**/**".format(
                            checkpoint_path, run_id, d, a, task
                        )
                    )
                )
                for f in files:
            ...
            if a == "moco_best-":
                files = sorted(
                    glob.glob("{}{}/{}/{}/{}_best_images/**/**".format(
                        checkpoint_path, run_id, d, a, task
                    ))
                )
                files_fiterrors = sorted(
                    glob.glob("{}{}/{}/{}/{}_best_fit_errors/**/**".format(
                        checkpoint_path, run_id, d, a, task
                    ))
                )
